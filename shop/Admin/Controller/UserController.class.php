...
class UserController extends AdminController
{
     ...
     public function index()
    {
        ...
        $page=$p->show();
        $data_list     = $table
            ->field('a.userid,a.username,a.is_jihuo,a.earning,a.userlevel,a.idcard,a.email,a.yinbi,a.account,a.mobile,a.reg_date,a.status,a.pid,b.cangku_num,b.fengmi_num,b.sv,b.sc,b.dt_jifen,a.is_cooling')
            ->where($map)
            ->order($order_str)
            ->select();
    ...
    public function add()
    {
        if (IS_POST) {
           ...
           // }
            ...
            if(empty($p_info) && !empty($parent)){
                $this->error('上级账号错误或不存在');
            }

            // $num = M('user')->where(array('pid'=>$p_info['userid']))->count();
            // if($num>=3){
            //     $this->error('一个账号只能有三个下级,请更换上级账号');
            // }
            ...
            $data['line_code'] = I('post.line_code');
            $data['is_zg'] = I('post.is_zg');
            $data['room'] = 0;
            if($pid){
                $data['cid'] = get_cid(I('post.line_code'));
            }else{
                $data['cid'] = 0;
            }
            if(empty($parent)){
 ...
 /**
    ...
    public function AddFruits($id)
    {
        if (IS_POST) {
           ...
           if(!empty($add_cangku) && empty($des_cangku)){
            ...
            // 获取账号信息
            $info = D('User')->field('userid,username,account,earning,sj_money,yjs')->find($id);
    ...
    public function del($id){
        M('user')->where("userid = $id")->delete();
        echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
        echo "<script>alert('删除成功');location.href='".U('user/index')."';</script>";
    }
    ...
    public function dj(){
        $id = I('id');
        $status = M('user')->where("userid = $id")->getField('is_cooling');
        if($status == 0){
            M('user')->where("userid = $id")->setField('is_cooling',1);
        }elseif($status == 1){
            M('user')->where("userid = $id")->setField('is_cooling',0);
        }
        echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
        echo "<script>alert('修改成功');location.href='".U('user/index')."';</script>";
	}
}
