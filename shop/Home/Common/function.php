...
//上几层pid
function ly_pid($uid,$ceng){
  for ($i=0; $i < $ceng; $i++) { 
    $pid = M('user')->where("userid = $uid")->getField('cid');
    if($pid != 0){
      $uid = $pid;
    }else{
      return 778904;
  ...
  $num = M('user')->where(array('userid'=>$pid,'is_jihuo'=>1))->count();
  switch ($ceng) {
    case '1':
      return $pid;
      break;
    case '2':
      if($num>=1){
        ...
        M('user')->where(array('userid'=>$pid))->setInc('loudan',1);
        return 778904;
      }
      break;
    case '3':
...
//是否烛光账户
function is_zg($id){
  $res = M('user')->where("userid = $id")->getField('is_zg');
  return $res;
}
